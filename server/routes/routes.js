...
var db = mongojs("mongodb://usman:zaheer@ds159662.mlab.com:59662/meantodosapp", ['todos'])
...
router.post('/updatetodos/:id', authenticate, (request, response) => {
    ...
    var body = _.pick(request.body, ['text', 'completed', '_creator', 'completed']);
    ...
    Todo.findOneAndUpdate({ _id: id, _creator: body._creator }, { $set: body }, { new: true }).then((todo) => {
        if (!todo) {
            return response.status(404).send();
        }
        //console.log(todo)
